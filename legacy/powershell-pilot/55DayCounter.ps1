...
$Script:AppVersion = if (Test-Path $Script:VersionPath) { (Get-Content -LiteralPath $Script:VersionPath -Raw).Trim() } else { "dev" }
...
function Use-StatusSelectionStyle {
    param(
...
function Refresh-Grid {
    ...
    foreach ($guest in $Script:Guests | Sort-Object Status, CheckOutDate, RoomNumber) {
        ...
        Use-StatusSelectionStyle -Row $row -Status $status
    }
...
function Get-TodaysListItems {
    ...
    foreach ($guest in $Script:Guests | Sort-Object CheckOutDate, RoomNumber) {
        ...
        $checkout = ([datetime]$guest.CheckOutDate).Date
        if ($checkout -gt $endDate) { continue }

        $daysLeft = ($checkout - $today).Days
        $daysText = if ($daysLeft -eq 0) {
            "Due today"
        }
        elseif ($daysLeft -lt 0) {
...
function Export-TodaysList {
    ...
    if (-not $Rows -or $Rows.Count -eq 0) {
        [System.Windows.Forms.MessageBox]::Show("There are no overdue guests or guests checking out today or in the next 5 days.", "Today's List") | Out-Null
        return
    }
...
function Show-TodaysListDialog {
    ...
    $title.Text = "Overdue guests, today, and the next 5 days"
    ...
    foreach ($row in $rows) {
        ...
        Use-StatusSelectionStyle -Row $previewGrid.Rows[$rowIndex] -Status $row.Status
    }
...
$Script:MainForm.Text = "55 Day Counter v$Script:AppVersion"
...
$helpLabel.Text = "Version $Script:AppVersion. Check-in is day 1. The 55th day is check-in + 54 days; alerts start 5 days before."
