...
$LogPath = Join-Path $env:TEMP "DaysCounter-uninstall.log"
$TaskNames = @(
    "DaysCounter Alerts",
...
Write-UninstallLog "Uninstalling $ProductName..."

try {
    Add-Type -AssemblyName System.Windows.Forms
    $choice = [System.Windows.Forms.MessageBox]::Show(
        "This will remove $ProductName, local guest data, shortcuts, and scheduled notifications. Continue?",
    ...
    $reply = Read-Host "Remove $ProductName and local data? Y/N"
...
foreach ($path in @($InstallDir, $LegacyInstallDir)) {
    if (Test-Path -LiteralPath $path) {
        ...
        Write-UninstallLog "Installed app folder removed: $path"
    }
}
...
try {
    ...
    [System.Windows.Forms.MessageBox]::Show(
        "$ProductName was uninstalled.`n`nLog: $LogPath",
        "$ProductName Uninstaller",
    ...
    Write-Host "$ProductName was uninstalled. Log: $LogPath"
}
