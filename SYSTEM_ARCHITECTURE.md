...
The app is built with Windows PowerShell and WinForms.

Main entry points:

- `55DayCounterInstaller.exe`: self-contained installer for hotel PCs.
- `Install-55DayCounter.cmd`: one-step installer for hotel PCs.
- `Install-55DayCounter.ps1`: copies app files, creates shortcuts, creates/preserves local data, and installs daily notifications.
- `Start-55DayCounter.cmd`: launches the desktop app.
- `55DayCounter.ps1`: main WinForms application.
- `Install-DailyNotifications.cmd`: installs a Windows Scheduled Task for daily notification checks.
- `Install-DailyNotifications.ps1`: creates or updates the scheduled task.
- `Check-55DayNotifications.ps1`: lightweight notification checker used by the scheduled task.
